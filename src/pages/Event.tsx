...
import { createEvent, fetchAllEvents } from "../service/event";
import EventBox from "../components/EventsComponent"
const getTodayDate = () => {
...
export default function Event() {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, setFile: React.Dispatch<React.SetStateAction<File | null>>, setPreviewUrl: React.Dispatch<React.SetStateAction<string | null>>) => {
    ...
    const handleEventCreate = async (e: React.FormEvent<HTMLButtonElement>) => {
        ...
        if (!eventName || !eventDescription || !category || !eventDate || !eventStartingTime || !eventCity || !eventLocation || !eventImageFile) {
            showAlert({ icon: "warning", title: "Incomplete Data", text: "Please fill in all required fields." });
            return;
        }
        ...
        if (eventDate === todayDate) {
            if (eventStartingTime <= currentTime) {
                showAlert({ icon: "error", title: "Invalid Time", text: "Event starting time must be in the future." });
                return;
            }
        }
        ...
        try {
            ...
            showAlert({ icon: "success", title: "Event Created", text: "The event has been created successfully." });
            setOpenModal(false);
            ...
            showAlert({ icon: "error", title: "Creation Failed", text: errorMessage });
        } finally {
            setIsLoading(false);
        }
    }
        ...
        <div className="w-full mt-[150px] container mx-auto">
            ...
            <Modal isOpen={openModal} onClose={() => setOpenModal(false)}>
                ...
                <form className="flex flex-col overflow-auto max-h-[70vh]">
                    ...
                    <label htmlFor="eventDescription" className="block text-gray-800 font-medium mb-1">
                ...
                <div className="flex justify-end space-x-4 mt-4">
                    <Button type="button" className="px-4 py-2 mt-5" color="red" onClick={handleEventModalResert} disabled={isLoading}>
                        Resert Form
                    </Button>
                    <Button type="submit" className="px-4 py-2 mt-5" color="green" onClick={handleEventCreate} disabled={isLoading}>
