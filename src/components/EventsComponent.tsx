...
import { fetchAllEvents } from '../service/event'; // API call to fetch events
import type { EventData } from '../service/event'; // Event data type
import Button from "../components/Button"; // Reusable Button component
import { Heart, MessageCircle, UserPlus, Share2 } from 'lucide-react'; // 💡 NEW: Lucide Icons
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg overflow-hidden transition-transform duration-300 hover:scale-[1.02] border border-gray-200">
            <div className="relative h-48">
                ...
                />
                <span className={`absolute top-2 right-2 text-xs font-semibold px-3 py-1 rounded-full text-white ${statusColor}`}>
                    {event.EventStatus}
                </span>
            </div>

            <div className="p-4">
                <h3 className="text-xl font-bold text-gray-800 mb-1 truncate">{event.category} - {event.eventName}</h3>
                <p className="text-sm text-gray-600 mb-3">{event.eventCity}, {event.eventLocation}</p>
                ...
                <p className="text-gray-500 text-sm mb-4 line-clamp-2">{event.eventDescription}</p>

                <div id='social' className="flex justify-between items-center space-x-4 pt-2 border-t border-gray-100">
...
export default function Events() {
    ...
    useEffect(() => {
        const loadEvents = async () => {
            ...
            try {
                ...
                const errorMessage = err.response?.data?.message || err.message || "Failed to fetch events from server.";
    ...
    if (isLoading) {
        ...
        content = <p className="text-lg text-red-500 mt-8">Error loading events: {error}</p>;
